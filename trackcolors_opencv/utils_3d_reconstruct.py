...
class Calibration:
        ...
        for f in file:
            ...
            if ret == True:
                corners2 = cv2.cornerSubPix(gray, corners, (5, 5), (-1, -1),
                                            self.criteria)  # (chessboard_size[0]*chessboard_size[1], 1, 2}
    ...
    def project_3Daxis_to_2Dimage(self, img, mat, dist, rvec, tvec):
        axis_3D = np.float32([[0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0,
                                                                1]]) * 2 * self.chessboard_sq_size  # Create a 3D axis that is twice the length of chessboard_sq_size
    ...
    def create_chessboard_pattern_open3d(self):
        ...
        for i in range(self.chessboard_size[1] + 1):  # In +ve Y axis direction
            ...
            if (self.chessboard_size[
                    0] + 1) % 2 == 0:  # Important: Need to check if col is even else will get parallel black strips as for even col the sq in the next row follw the same color
                black = not black
